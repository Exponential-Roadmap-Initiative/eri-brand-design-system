...
export const SKILLS_METADATA: SkillMeta[] = [
  // ── Tier 1: Always-on ──────────────────────────────────────────────────────
  {
    ...
    description: "Tier 1 orchestrator for the ERI skills library. Read at the start of every task to identify which skills apply. Contains a task-type lookup table: match the user's request to a task type, then read ALL skills listed for that type before proceeding. Replaces the mechanical trigger list with task-first reasoning. Also contains the mandatory post-task usage log instruction. The skill-set mappings section is auto-generated by the BDS sync button — do not edit it manually. Version: 1.1.0",
    tier: 1,
    category: "process",
    version: "1.1.0",
  ...
  },
  {
    ...
    description: "Compact always-on ERI brand reference. Use before any ERI product work. Covers: system operations checklist (Steps 0–2), pre-action decision table, canonical colour tokens, typography (GDPR self-hosted WOFF2 only), navigation tier decision, hero layout rules, dark mode enforcement (CSS token block + semantic token table), CTA/language/anti-AI design patterns, project alignment checklist (S/T/B/C/A), bds-meta.json schema pointer, and BDS section index. For full component prop tables, install instructions, overlay implementation, and hero code examples, read eri-bds-components. Version: 4.0.0",
    tier: 1,
    category: "brand",
    version: "4.0.0",
  ...
  },
  {
    ...
    description: "Full component prop reference for @eri/components. Covers: install + version auto-sync, all six standard components (EriPageLayout, EriAppHeader, EriAppFooter, EriHeroSection, EriStatusBadge, EriContactUsButton) with full prop tables, integration notes, canonical App.tsx pattern, nav overlay implementation (NavDrawer.tsx), hero section manual fallback, web app header anatomy, favicon spec, hero images CDN table, key CDN asset URLs, footer standard, Contact Us integration (URL pattern, button, registered source IDs), cross-site theme system (ThemeContext.tsx verbatim, FOLC script), layout wrapper pattern, and bds-meta.json full schema. Version: 1.0.0",
    tier: 2,
    category: "brand",
    version: "1.0.0",
...
export const skillsRouter = router({
  ...
  syncMetadataFromFiles: adminProcedure.mutation(async () => {
    ...
    for (const dirName of skillDirs) {
      ...
      // Skip skills marked as retired in their frontmatter
      try {
        const checkContent = fs.readFileSync(path.join(skillsDir, dirName, "SKILL.md"), "utf-8");
        const retiredMatch = checkContent.match(/^---[\s\S]*?retired:\s*true[\s\S]*?---/m);
        if (retiredMatch) continue; // skip retired skills
    ...
    // ── Write back SKILLS_METADATA if anything changed ──────────────────────────
    if (changes.length > 0 || registered.length > 0) {
      fs.writeFileSync(filePath, updatedSource, "utf-8");
    }

    // ── Sync orchestrator skill index ─────────────────────────────────────────
    // Re-read SKILLS_METADATA from the updated source to get the latest state
    // and regenerate the <!-- ORCHESTRATOR_SKILLS_START/END --> block in
    // eri-skills-orchestrator/SKILL.md
    try {
      const orchestratorPath = path.join(skillsDir, "eri-skills-orchestrator", "SKILL.md");
      if (fs.existsSync(orchestratorPath)) {
        // Build tier index from the in-memory SKILLS_METADATA (updated entries
        // will be reflected on next server restart; use the source-of-truth array
        ...
        if (orchStart !== -1 && orchEnd !== -1) {
          orchContent =
            orchContent.slice(0, orchStart) +
            newBlock +
            orchContent.slice(orchEnd + "<!-- ORCHESTRATOR_SKILLS_END -->".length);
          fs.writeFileSync(orchestratorPath, orchContent, "utf-8");
        }
      }
    } catch {
      // Non-fatal — orchestrator sync failure does not block the main sync
    }
