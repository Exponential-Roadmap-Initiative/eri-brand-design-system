...
export const SKILLS_METADATA: SkillMeta[] = [
  // ── Tier 1: Always-on ──────────────────────────────────────────────────────
  {
    ...
    description: "Tier 1 orchestrator for the ERI skills library. Read at the start of every task to identify which skills apply. Contains a task-type lookup table: match the user's request to a task type, then read ALL skills listed for that type before proceeding. Replaces the mechanical trigger list with task-first reasoning. Also contains the mandatory post-task usage log instruction. The skill-set mappings section is auto-generated by the BDS sync button -- do not edit it manually. Version: 1.0.0",
    tier: 1,
    category: "process",
    version: "1.0.0",
