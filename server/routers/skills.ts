...
export const SKILLS_METADATA: SkillMeta[] = [
  ...
  },
  {
    ...
    description: "Compact always-on ERI brand reference. Use before any ERI product work — new builds, updates, fixes, or audits of any ERI app. Covers: system operations checklist (Steps 0–2), pre-action decision table, canonical colour tokens, typography (GDPR self-hosted WOFF2 only), navigation tier decision, hero layout rules, dark mode enforcement (semantic token table + anti-patterns), CTA rules, language rules, anti-AI design patterns, project alignment checklist (S/T/B/C/A), and bds-meta.json pointer. For full component prop tables, install instructions, overlay implementation, and code examples, read eri-bds-components. Version: 4.2.0",
    tier: 1,
    category: "brand",
    version: "4.2.0",
